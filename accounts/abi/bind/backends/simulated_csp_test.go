...
func TestSimulatedBackendCSP(t *testing.T) {
	...
	tx, _ = types.SignTx[csp.Cert, csp.PublicKey](tx, types.HomesteadSigner[csp.PublicKey]{}, crt)
...
func TestSimulatedBackendCSPtoGOST3410(t *testing.T) {
	...
	t.Log("Nonce", nonce)
	balanceCheck, err := sim.BalanceAt(context.Background(), addr, nil)
	if err != nil {
		t.Error(err)
	}
	t.Log("Balance", balanceCheck)

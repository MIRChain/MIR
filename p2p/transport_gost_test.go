...
func TestProtocolHandshakeGost(t *testing.T) {
	var (
		prv0, _ = crypto.GenerateKey[gost3410.PrivateKey]()
		pub0    = crypto.FromECDSAPub[gost3410.PublicKey](*prv0.Public())[1:]
		hs0     = &protoHandshake{Version: 3, ID: pub0, Caps: []Cap{{"a", 0}, {"b", 2}}}

		prv1, _ = crypto.GenerateKey[gost3410.PrivateKey]()
		pub1    = crypto.FromECDSAPub[gost3410.PublicKey](*prv1.Public())[1:]
	...
	go func() {
		...
		if !reflect.DeepEqual(rpubkey, *prv1.Public()) {
			t.Errorf("dial side remote pubkey mismatch: got %v, want %v", rpubkey, *prv1.Public())
			return
		}
	...
	go func() {
		...
		if !reflect.DeepEqual(rpubkey, *prv0.Public()) {
			t.Errorf("listen side remote pubkey mismatch: got %v, want %v", rpubkey, *prv0.Public())
			return
		}
...
func TestProtocolHandshakeErrorsGost(t *testing.T) {
	tests := []struct {
		code uint64
		msg  interface{}
		err  error

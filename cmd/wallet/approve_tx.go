...
import (
	...
	core2 "github.com/pavelkrolevets/MIR-pro/signer/core"
...
type ApproveTxCtx struct {
	...
	answerCh chan (int)
	formData core2.SendTxArgs[gost3410.PublicKey]
	ClefUI   *ClefUI
	rawTx    core2.SendTxArgs[gost3410.PublicKey]
}

func (t *ApproveTxCtx) init() {
	t.formData = core2.SendTxArgs[gost3410.PublicKey]{}
	t.SetValueUnit(GWEI)
	t.SetGasPriceUnit(GWEI)
}

func (t *ApproveTxCtx) SetTransaction(tx core2.SendTxArgs[gost3410.PublicKey]) {
	...
	t.SetNonce(fmt.Sprintf("%d", tx.Nonce))
	{
		...
		t.SetValueUnit(GWEI)
		t.SetValue(WeiToString(&bigV, GWei))
	}
...

	{
		...
		t.SetGasPriceUnit(GWEI)
		t.SetGasPrice(WeiToString(&bigGas, GWei))
	}
	t.SetFrom(tx.From.Original())
	t.SetFromSrc(ToBase64Img(tx.From.Address()))

	if tx.To != nil {
		t.SetTo(tx.To.Original())
		t.SetToSrc(ToBase64Img(tx.To.Address()))
	}
}
...
func (t *ApproveTxCtx) changeGasPriceUnit(toIndex int) {
	...
	weiVal, err := TextToWei(t.GasPrice(), fromUnit)
	if err != nil {
...
func (t *ApproveTxCtx) getNewTx() (core2.SendTxArgs[gost3410.PublicKey], error) {
	tx := core2.SendTxArgs[gost3410.PublicKey]{}

	// Gasprice
	if gp, err := TextToWei(t.GasPrice(), indexToUnit(t.GasPriceUnit())); err != nil {
		return tx, err
	} else {
		tx.GasPrice = hexutil.Big(*gp)
	}
	// Value
	if v, err := TextToWei(t.Value(), indexToUnit(t.ValueUnit())); err != nil {
		return tx, err
	} else {
...
func (t *ApproveTxCtx) ClickResponse(responseCh chan *core2.SignTxResponse[gost3410.PublicKey]) {
	go func() {
		answer := <-t.answerCh
		if answer == -1 { // Go back
			t.Reset()
		...
		var reply = new(core2.SignTxResponse[gost3410.PublicKey])
		if answer == 1 { // Reject
...
func compareTransactions(o core2.SendTxArgs[gost3410.PublicKey], n core2.SendTxArgs[gost3410.PublicKey]) []diff {
	var diffs []diff

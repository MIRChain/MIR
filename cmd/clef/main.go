...
var (
	...
	chainIdFlag = cli.Int64Flag{
		Name:  "chainid",
		Value: params.MainnetMirChainConfig.ChainID.Int64(),
	...
	initCommand = cli.Command{
		Action:    utils.MigrateFlags(_initializeSecrets),
		Name:      "init",
...
the rule-engine to work.`,
	...
	attestCommand = cli.Command{
		Action:    utils.MigrateFlags(_attestFile),
		Name:      "attest",
...
Clef that the file is 'safe' to execute.`,
	...
	setCredentialCommand = cli.Command{
		Action:    utils.MigrateFlags(_setCredential),
		Name:      "setpw",
...
`}
	delCredentialCommand = cli.Command{
		Action:    utils.MigrateFlags(_removeCredential),
		Name:      "delpw",
...
`}
	newAccountCommand = cli.Command{
		Action:    utils.MigrateFlags(_newAccount),
		Name:      "newaccount",
...
func init() {
	app.Name = "Clef"
	app.Usage = "Manage Mir account operations"
	app.Flags = []cli.Flag{
		logLevelFlag,
		keystoreFlag,
		configdirFlag,
...
func _initializeSecrets(ctx *cli.Context) error {
	...
	if ctx.GlobalString(utils.CryptoSwitchFlag.Name) != "" {
		if ctx.GlobalString(utils.CryptoSwitchFlag.Name) == "gost" {
			...
			err := initializeSecrets[gost3410.PrivateKey, gost3410.PublicKey](ctx)
			if err != nil {
				return err
			}
			...
			║  ├┬┘└┬┘├─┘ │ │ │  ║║║║╚═╗ ║ 
			╚═╝┴└─ ┴ ┴   ┴ └─┘  ╝╚╝╩╚═╝ ╩ `)
			err := initializeSecrets[nist.PrivateKey, nist.PublicKey](ctx)
			if err != nil {
				return err
			}
...
func initializeSecrets[T crypto.PrivateKey, P crypto.PublicKey](c *cli.Context) error {
	...
	cipherSeed, err := encryptSeed[P](masterSeed, []byte(password), n, p)
	if err != nil {
...
func _attestFile(ctx *cli.Context) error {
	...
	if ctx.GlobalString(utils.CryptoSwitchFlag.Name) != "" {
		if ctx.GlobalString(utils.CryptoSwitchFlag.Name) == "gost" {
			...
			err := attestFile[gost3410.PrivateKey, gost3410.PublicKey](ctx)
			if err != nil {
				return err
			}
		} else if ctx.GlobalString(utils.CryptoSwitchFlag.Name) == "gost_csp" {
			...
			║  ├┬┘└┬┘├─┘ │ │ │  ║║║║╚═╗ ║ 
			╚═╝┴└─ ┴ ┴   ┴ └─┘  ╝╚╝╩╚═╝ ╩ `)
			err := attestFile[nist.PrivateKey, nist.PublicKey](ctx)
			if err != nil {
				return err
			}
		} else if ctx.GlobalString(utils.CryptoSwitchFlag.Name) == "pqc" {
...
func attestFile[T crypto.PrivateKey, P crypto.PublicKey](ctx *cli.Context) error {
	...
	vaultLocation := filepath.Join(configDir, common.Bytes2Hex(crypto.Keccak256[P]([]byte("vault"), stretchedKey)[:10]))
	confKey := crypto.Keccak256[P]([]byte("config"), stretchedKey)
...
func _setCredential(ctx *cli.Context) error {
	...
	if ctx.GlobalString(utils.CryptoSwitchFlag.Name) != "" {
		if ctx.GlobalString(utils.CryptoSwitchFlag.Name) == "gost" {
			...
			err := setCredential[gost3410.PrivateKey, gost3410.PublicKey](ctx)
			if err != nil {
				return err
			}
		} else if ctx.GlobalString(utils.CryptoSwitchFlag.Name) == "gost_csp" {
			...
			║  ├┬┘└┬┘├─┘ │ │ │  ║║║║╚═╗ ║ 
			╚═╝┴└─ ┴ ┴   ┴ └─┘  ╝╚╝╩╚═╝ ╩ `)
			err := setCredential[nist.PrivateKey, nist.PublicKey](ctx)
			if err != nil {
				return err
			}
		} else if ctx.GlobalString(utils.CryptoSwitchFlag.Name) == "pqc" {
...
func setCredential[T crypto.PrivateKey, P crypto.PublicKey](ctx *cli.Context) error {
	...
	configDir := ctx.GlobalString(configdirFlag.Name)
	vaultLocation := filepath.Join(configDir, common.Bytes2Hex(crypto.Keccak256[P]([]byte("vault"), stretchedKey)[:10]))
	pwkey := crypto.Keccak256[P]([]byte("credentials"), stretchedKey)
...
func _removeCredential(ctx *cli.Context) error {
	...
	if ctx.GlobalString(utils.CryptoSwitchFlag.Name) != "" {
		if ctx.GlobalString(utils.CryptoSwitchFlag.Name) == "gost" {
			...
			err := removeCredential[gost3410.PrivateKey, gost3410.PublicKey](ctx)
			if err != nil {
				return err
			}
		} else if ctx.GlobalString(utils.CryptoSwitchFlag.Name) == "gost_csp" {
			...
			║  ├┬┘└┬┘├─┘ │ │ │  ║║║║╚═╗ ║ 
			╚═╝┴└─ ┴ ┴   ┴ └─┘  ╝╚╝╩╚═╝ ╩ `)
			err := removeCredential[nist.PrivateKey, nist.PublicKey](ctx)
			if err != nil {
				return err
			}
		} else if ctx.GlobalString(utils.CryptoSwitchFlag.Name) == "pqc" {
...
func clef(ctx *cli.Context) error {
	fmt.Println(`
	███╗   ███╗██╗██████╗      ██████╗██╗  ██╗ █████╗ ██╗███╗   ██╗
	████╗ ████║██║██╔══██╗    ██╔════╝██║  ██║██╔══██╗██║████╗  ██║
	██╔████╔██║██║██████╔╝    ██║     ███████║███████║██║██╔██╗ ██║
...
func signer[T crypto.PrivateKey, P crypto.PublicKey](c *cli.Context) error {
	...
	db, err := fourbyte.NewWithFile[P](fourByteLocal)
	if err != nil {
		utils.Fatalf(err.Error())
	}
	...
	if stretchedKey, err := readMasterKey(c, ui); err != nil {
		...
		pwkey := crypto.Keccak256[P]([]byte("credentials"), stretchedKey)
		jskey := crypto.Keccak256[P]([]byte("jsstorage"), stretchedKey)
		confkey := crypto.Keccak256[P]([]byte("config"), stretchedKey)
...
func readMasterKey[T crypto.PrivateKey, P crypto.PublicKey](ctx *cli.Context, ui core.UIClientAPI[T,P]) ([]byte, error) {
	...
	masterSeed, err := decryptSeed[P](cipherKey, password)
	if err != nil {
	...
	vaultLocation := filepath.Join(configDir, common.Bytes2Hex(crypto.Keccak256[P]([]byte("vault"), masterSeed)[:10]))
...
func testExternalUI[T crypto.PrivateKey, P crypto.PublicKey](api *core.SignerAPI[T,P]) {
	...
	{ // Sign data test - typed data
		...
		var typedData core.TypedData[P]
...
func encryptSeed[P crypto.PublicKey](seed []byte, auth []byte, scryptN, scryptP int) ([]byte, error) {
	cryptoStruct, err := keystore.EncryptDataV3[P](seed, auth, scryptN, scryptP)
	if err != nil {
		return nil, err
	}
...
func decryptSeed[P crypto.PublicKey](keyjson []byte, auth string) ([]byte, error) {
	...
	seed, err := keystore.DecryptDataV3[P](encSeed.Params, auth)
	if err != nil {
		return nil, err
	}
	return seed, err
...
func GenDoc[T crypto.PrivateKey, P crypto.PublicKey](ctx *cli.Context) {
	...
	{ // Sign transaction request
		...
		add("SignTxRequest", desc, &core.SignTxRequest[P]{
			Meta: meta,
	...
	{ // Sign tx response
		...
		add("SignTxResponse - approve", "Response to request to sign a transaction. This response needs to contain the `transaction`"+
			...
			&core.SignTxResponse[P]{Approved: true,
				Transaction: core.SendTxArgs[P]{
					Data:     &data,
					Nonce:    0x4,
					Value:    hexutil.Big(*big.NewInt(6)),
	...
	{ // WHen a signed tx is ready to go out
		...
		add("OnApproved - SignTransactionResult", desc, &ethapi.SignTransactionResult[P]{Raw: rlpdata, Tx: &tx})

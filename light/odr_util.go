...
func GetHeaderByNumber[P crypto.PublicKey](ctx context.Context, odr OdrBackend[P], number uint64) (*types.Header[P], error) {
	...
	if (hash != common.Hash{}) {
		if header := rawdb.ReadHeader[P](db, hash, number); header != nil {
			return header, nil
		}
	}
...
func GetCanonicalHash[P crypto.PublicKey](ctx context.Context, odr OdrBackend[P], number uint64) (common.Hash, error) {
...
func GetTd[P crypto.PublicKey](ctx context.Context, odr OdrBackend[P], hash common.Hash, number uint64) (*big.Int, error) {
...
func GetBodyRLP[P crypto.PublicKey](ctx context.Context, odr OdrBackend[P], hash common.Hash, number uint64) (rlp.RawValue, error) {
	...
	r := &BlockRequest[P]{Hash: hash, Number: number, Header: header}
	if err := odr.Retrieve(ctx, r); err != nil {
		return nil, err
	}
	return r.Rlp, nil
...
// hash.
func GetBody[P crypto.PublicKey](ctx context.Context, odr OdrBackend[P], hash common.Hash, number uint64) (*types.Body[P], error) {
...
func GetBlock[P crypto.PublicKey](ctx context.Context, odr OdrBackend[P], hash common.Hash, number uint64) (*types.Block[P], error) {
	// Retrieve the block header and body contents
	header, err := GetHeaderByNumber(ctx, odr, number)
	if err != nil {
		return nil, errNoHeader
	}
	body, err := GetBody[P](ctx, odr, hash, number)
	if err != nil {
		return nil, err
	}
	// Reassemble the block and return
	return types.NewBlockWithHeader[P](header).WithBody(body.Transactions, body.Uncles), nil
}

// GetBlockReceipts retrieves the receipts generated by the transactions included
// in a block given by its hash.
func GetBlockReceipts[P crypto.PublicKey](ctx context.Context, odr OdrBackend[P], hash common.Hash, number uint64) (types.Receipts[P], error) {
	// Assume receipts are already stored locally and attempt to retrieve.
	receipts := rawdb.ReadRawReceipts[P](odr.Database(), hash, number)
	if receipts == nil {
		header, err := GetHeaderByNumber(ctx, odr, number)
		if err != nil {
			return nil, errNoHeader
		...
		r := &ReceiptsRequest[P]{Hash: hash, Number: number, Header: header}
		if err := odr.Retrieve(ctx, r); err != nil {
			return nil, err
		}
		receipts = r.Receipts
	...
	if len(receipts) > 0 && receipts[0].TxHash == (common.Hash{}) {
		block, err := GetBlock[P](ctx, odr, hash, number)
		if err != nil {
			return nil, err
		}
...
func GetBlockLogs[P crypto.PublicKey](ctx context.Context, odr OdrBackend[P], hash common.Hash, number uint64) ([][]*types.Log, error) {
	// Retrieve the potentially incomplete receipts from disk or network
	receipts, err := GetBlockReceipts[P](ctx, odr, hash, number)
	if err != nil {
		return nil, err
	}
...
func GetUntrustedBlockLogs[P crypto.PublicKey](ctx context.Context, odr OdrBackend[P], header *types.Header[P]) ([][]*types.Log, error) {
	// Retrieve the potentially incomplete receipts from disk or network
	hash, number := header.Hash(), header.Number.Uint64()
	receipts := rawdb.ReadRawReceipts[P](odr.Database(), hash, number)
	if receipts == nil {
		r := &ReceiptsRequest[P]{Hash: hash, Number: number, Header: header, Untrusted: true}
		if err := odr.Retrieve(ctx, r); err != nil {
			return nil, err
		}
		receipts = r.Receipts
...
func GetBloomBits[P crypto.PublicKey](ctx context.Context, odr OdrBackend[P], bit uint, sections []uint64) ([][]byte, error) {
	var (
		reqIndex    []int
		reqSections []uint64
		db          = odr.Database()
...
func GetTransaction[P crypto.PublicKey](ctx context.Context, odr OdrBackend[P], txHash common.Hash) (*types.Transaction[P], common.Hash, uint64, uint64, error) {
	...
	body, err := GetBody[P](ctx, odr, pos.BlockHash, pos.BlockIndex)

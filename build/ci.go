...
import (
	...
	"github.com/pavelkrolevets/MIR-pro/crypto/signify"
	"github.com/pavelkrolevets/MIR-pro/internal/build"
	"github.com/pavelkrolevets/MIR-pro/params"
)

var (
	// Files that end up in the geth*.zip archive.
	mirArchiveFiles = []string{
		"COPYING",
		executablePath("mir"),
	}
	...
	debExecutables = []debExecutable{
		...
		{
			BinaryName:  "mir",
			Description: "Ethereum-MIR CLI client.",
		},
		{
			BinaryName:  "puppeth",
...
func doArchive(cmdline []string) {
	...
	var (
		env      = build.Env()
		basemir = archiveBasename(*arch, params.ArchiveVersion(env.Commit))
		mir     = "mir-" + basemir + ext
		alltools = "mir-alltools-" + basemir + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(mir, mirArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{mir, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with mir-release.pub"
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "mir-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "mir-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archives (usually "mirstore/builds")`)
		workdir = flag.String("workdir", "", `Output directory for packages (uses temp dir if unset)`)
	)
	flag.CommandLine.Parse(cmdline)
	*workdir = makeWorkdir(*workdir)
	...
	build.Render("build/nsis.mir.nsi", filepath.Join(*workdir, "mir.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("mir-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archive (usually "mirstore/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	tc := new(build.GoToolchain)
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.ethereum", "-v", "github.com/pavelkrolevets/MIR-pro/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("mir.aar", filepath.Join(GOBIN, "mir.aar"))
		os.Rename("mir-sources.jar", filepath.Join(GOBIN, "mir-sources.jar"))
		return
	}
	...
	archive := "mir-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("mir.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/pavelkrolevets/MIR-pro/mobile")
	...
	// Create the archive.
	maybeSkipArchive(env)
	archive := "mir-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "Mir.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "Mir.podspec", "--allow-warnings")
	}
}
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "mirstore/builds")`)

...
func testHeaderConcurrentVerification(t *testing.T, threads int) {
	...
	for i, valid := range []bool{true, false} {
		...
		if valid {
			...
			chain, _ := NewBlockChain[nist.PublicKey](testdb, nil, params.TestChainConfig, ethash.NewFakeFailer[nist.PublicKey](uint64(len(headers)-1)), vm.Config[nist.PublicKey]{}, nil, nil, nil)
			_, results = chain.engine.VerifyHeaders(chain, headers, seals)
			chain.Stop()
		}

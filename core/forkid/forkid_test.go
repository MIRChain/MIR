...
func TestCreation(t *testing.T) {
	...
	tests := []struct {
		...
		// Mainnet test cases
		{
			params.MainnetEthChainConfig,
			params.MainnetEthGenesisHash,
			[]testcase{
		...
		// Soyuz test cases
		{
			params.SoyuzChainConfig,
			params.SoyuzGenesisHash,
			[]testcase{
...
func TestValidation(t *testing.T) {
	...
	for i, tt := range tests {
		filter := newFilter(params.MainnetEthChainConfig, params.MainnetEthGenesisHash, func() uint64 { return tt.head })

...
func TestGeneration(t *testing.T) {
	...
	stTrie, _ := trie.NewSecure[nist.PublicKey](common.Hash{}, triedb)
	...
	accTrie, _ := trie.NewSecure[nist.PublicKey](common.Hash{}, triedb)
	acc := &Account{Balance: big.NewInt(1), Root: stTrie.Hash().Bytes(), CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()}
	val, _ := rlp.EncodeToBytes(acc)
	accTrie.Update([]byte("acc-1"), val) // 0x9250573b9c18c664139f3b6a7a8081b7d8f8916a8fcc5d94feec6c29f5fd4e9e

	acc = &Account{Balance: big.NewInt(2), Root: emptyRoot.Bytes(), CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()}
	val, _ = rlp.EncodeToBytes(acc)
	accTrie.Update([]byte("acc-2"), val) // 0x65145f923027566669a1ae5ccac66f945b55ff6eaeb17d2ea8e048b7d381f2d7

	acc = &Account{Balance: big.NewInt(3), Root: stTrie.Hash().Bytes(), CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()}
	val, _ = rlp.EncodeToBytes(acc)
	...
	snap := generateSnapshot[nist.PublicKey](diskdb, triedb, 16, root)
	select {
...
func checkSnapRoot[P crypto.PublicKey](t *testing.T, snap *diskLayer[P], trieRoot common.Hash) {
	t.Helper()
	accIt := snap.AccountIterator(common.Hash{})
	defer accIt.Release()
	snapRoot, err := generateTrieRoot[P](nil, accIt, common.Hash{}, stackTrieGenerate[P],
		func(db ethdb.KeyValueWriter, accountHash, codeHash common.Hash, stat *generateStats) (common.Hash, error) {
			...
			hash, err := generateTrieRoot[P](nil, storageIt, accountHash, stackTrieGenerate[P], nil, stat, false)
			if err != nil {
...
func newHelper[P crypto.PublicKey]() *testHelper[P] {
	diskdb := memorydb.New()
	triedb := trie.NewDatabase(diskdb)
	accTrie, _ := trie.NewSecure[P](common.Hash{}, triedb)
	return &testHelper[P]{
		diskdb:  diskdb,
		triedb:  triedb,
		accTrie: accTrie,
...
func (t *testHelper[P]) addTrieAccount(acckey string, acc *Account) {
	val, _ := rlp.EncodeToBytes(acc)
	t.accTrie.Update([]byte(acckey), val)
}

func (t *testHelper[P]) addSnapAccount(acckey string, acc *Account) {
	val, _ := rlp.EncodeToBytes(acc)
	key := hashData([]byte(acckey))
	rawdb.WriteAccountSnapshot(t.diskdb, key, val)
}

func (t *testHelper[P]) addAccount(acckey string, acc *Account) {
	t.addTrieAccount(acckey, acc)
	t.addSnapAccount(acckey, acc)
}

func (t *testHelper[P]) addSnapStorage(accKey string, keys []string, vals []string) {
...
func (t *testHelper[P]) makeStorageTrie(keys []string, vals []string) []byte {
	stTrie, _ := trie.NewSecure[P](common.Hash{}, t.triedb)
	for i, k := range keys {
...
func (t *testHelper[P]) Generate() (common.Hash, *diskLayer[P]) {
	root, _ := t.accTrie.Commit(nil)
	t.triedb.Commit(root, false, nil)
	snap := generateSnapshot[P](t.diskdb, t.triedb, 16, root)
	return root, snap
}
...
func TestGenerateExistentStateWithWrongStorage(t *testing.T) {
	helper := newHelper[nist.PublicKey]()
	stRoot := helper.makeStorageTrie([]string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"})

	// Account one, empty root but non-empty database
	helper.addAccount("acc-1", &Account{Balance: big.NewInt(1), Root: emptyRoot.Bytes(), CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()})
	helper.addSnapStorage("acc-1", []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"})

	// Account two, non empty root but empty database
	helper.addAccount("acc-2", &Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()})

	// Miss slots
	{
		// Account three, non empty root but misses slots in the beginning
		helper.addAccount("acc-3", &Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()})
		helper.addSnapStorage("acc-3", []string{"key-2", "key-3"}, []string{"val-2", "val-3"})

		// Account four, non empty root but misses slots in the middle
		helper.addAccount("acc-4", &Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()})
		helper.addSnapStorage("acc-4", []string{"key-1", "key-3"}, []string{"val-1", "val-3"})

		// Account five, non empty root but misses slots in the end
		helper.addAccount("acc-5", &Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()})
	...
	// Wrong storage slots
	{
		...
		helper.addAccount("acc-6", &Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()})
		helper.addSnapStorage("acc-6", []string{"key-1", "key-2", "key-3"}, []string{"badval-1", "val-2", "val-3"})

		// Account seven, non empty root but wrong slots in the middle
		helper.addAccount("acc-7", &Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()})
		helper.addSnapStorage("acc-7", []string{"key-1", "key-2", "key-3"}, []string{"val-1", "badval-2", "val-3"})

		// Account eight, non empty root but wrong slots in the end
		helper.addAccount("acc-8", &Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()})
		helper.addSnapStorage("acc-8", []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "badval-3"})

		// Account 9, non empty root but rotated slots
		helper.addAccount("acc-9", &Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()})
	...
	// Extra storage slots
	{
		...
		helper.addAccount("acc-10", &Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()})
		helper.addSnapStorage("acc-10", []string{"key-0", "key-1", "key-2", "key-3"}, []string{"val-0", "val-1", "val-2", "val-3"})

		// Account 11, non empty root but extra slots in the middle
		helper.addAccount("acc-11", &Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()})
		helper.addSnapStorage("acc-11", []string{"key-1", "key-2", "key-2-1", "key-3"}, []string{"val-1", "val-2", "val-2-1", "val-3"})

		// Account 12, non empty root but extra slots in the end
		helper.addAccount("acc-12", &Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()})
...
func TestGenerateExistentStateWithWrongAccounts(t *testing.T) {
	...
	{
		helper.addTrieAccount("acc-1", &Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()}) // Beginning
		helper.addTrieAccount("acc-4", &Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()}) // Middle
		helper.addTrieAccount("acc-6", &Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()}) // End
	}

	// Wrong accounts
	{
		helper.addTrieAccount("acc-2", &Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()})
		helper.addSnapAccount("acc-2", &Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: common.Hex2Bytes("0x1234")})

		helper.addTrieAccount("acc-3", &Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()})
		helper.addSnapAccount("acc-3", &Account{Balance: big.NewInt(1), Root: emptyRoot.Bytes(), CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()})
	}
...
func TestGenerateCorruptAccountTrie(t *testing.T) {
	...
	tr, _ := trie.NewSecure[nist.PublicKey](common.Hash{}, triedb)
	acc := &Account{Balance: big.NewInt(1), Root: emptyRoot.Bytes(), CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()}
	val, _ := rlp.EncodeToBytes(acc)
	...
	acc = &Account{Balance: big.NewInt(3), Root: emptyRoot.Bytes(), CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()}
	val, _ = rlp.EncodeToBytes(acc)
	...
	snap := generateSnapshot[nist.PublicKey](diskdb, triedb, 16, common.HexToHash("0xa04693ea110a31037fb5ee814308a6f1d76bdab0b11676bdf4541d2de55ba978"))
	select {
	case <-snap.genPending:
		// Snapshot generation succeeded
...
func TestGenerateMissingStorageTrie(t *testing.T) {
	...
	snap := generateSnapshot[nist.PublicKey](diskdb, triedb, 16, common.HexToHash("0xe3712f1a226f3782caca78ca770ccc19ee000552813a9f59d479f8611db9b1fd"))
	select {
	case <-snap.genPending:
		// Snapshot generation succeeded
...
func getStorageTrie[P crypto.PublicKey](n int, triedb *trie.Database) *trie.SecureTrie[P] {
	stTrie, _ := trie.NewSecure[P](common.Hash{}, triedb)
	for i := 0; i < n; i++ {
...
func TestGenerateWithExtraAccounts(t *testing.T) {
	var (
		diskdb = memorydb.New()
		triedb = trie.NewDatabase(diskdb)
		stTrie = getStorageTrie[nist.PublicKey](5, triedb)
	)
	accTrie, _ := trie.NewSecure[nist.PublicKey](common.Hash{}, triedb)
...
func TestGenerateWithManyExtraAccounts(t *testing.T) {
	...
	var (
		diskdb = memorydb.New()
		triedb = trie.NewDatabase(diskdb)
		stTrie = getStorageTrie[nist.PublicKey](3, triedb)
	)
	accTrie, _ := trie.NewSecure[nist.PublicKey](common.Hash{}, triedb)
	{ // Account one in the trie
		acc := &Account{Balance: big.NewInt(1), Root: stTrie.Hash().Bytes(), CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()}
	...
	{ // 100 accounts exist only in snapshot
		for i := 0; i < 1000; i++ {
			//acc := &Account{Balance: big.NewInt(int64(i)), Root: stTrie.Hash().Bytes(), CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()}
			acc := &Account{Balance: big.NewInt(int64(i)), Root: emptyRoot.Bytes(), CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()}
			val, _ := rlp.EncodeToBytes(acc)
...
func TestGenerateWithExtraBeforeAndAfter(t *testing.T) {
	...
	accTrie, _ := trie.New[nist.PublicKey](common.Hash{}, triedb)
	{
		acc := &Account{Balance: big.NewInt(1), Root: emptyRoot.Bytes(), CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()}
		val, _ := rlp.EncodeToBytes(acc)
...
func TestGenerateFromEmptySnap(t *testing.T) {
	...
	for i := 0; i < 400; i++ {
		helper.addTrieAccount(fmt.Sprintf("acc-%d", i),
			&Account{Balance: big.NewInt(1), Root: stRoot, CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()})
	}
	root, snap := helper.Generate()
...
func TestGenerateWithIncompleteStorage(t *testing.T) {
	...
	for i := 0; i < 8; i++ {
		...
		helper.addAccount(accKey, &Account{Balance: big.NewInt(int64(i)), Root: stRoot, CodeHash: crypto.Keccak256Hash[nist.PublicKey](nil).Bytes()})

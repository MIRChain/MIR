...
func TestChainIterator(t *testing.T) {
	...
	block = types.NewBlock[nist.PublicKey](&types.Header[nist.PublicKey]{Number: big.NewInt(int64(0))}, nil, nil, nil, newHasher()) // Empty genesis block
	...
	for i := uint64(1); i <= 10; i++ {
		...
		txs = append(txs, tx)
		block = types.NewBlock[nist.PublicKey](&types.Header[nist.PublicKey]{Number: big.NewInt(int64(i))}, []*types.Transaction[nist.PublicKey]{tx}, nil, nil, newHasher())
		WriteBlock(chainDb, block)
		WriteCanonicalHash(chainDb, block.Hash(), block.NumberU64())
	}
...
func TestIndexTransactions(t *testing.T) {
	...
	block = types.NewBlock[nist.PublicKey](&types.Header[nist.PublicKey]{Number: big.NewInt(int64(0))}, nil, nil, nil, newHasher())
	WriteBlock(chainDb, block)
	WriteCanonicalHash(chainDb, block.Hash(), block.NumberU64())

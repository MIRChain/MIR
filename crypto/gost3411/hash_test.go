...
func TestVectors(t *testing.T) {
	...
	t.Run("1", func(t *testing.T) {
		...
		h512.Write(m)
		t.Log(hex.EncodeToString(h512.Sum(nil)))
	...
	t.Run("Се ветри", func(t *testing.T) {
		...
		res := make([]byte, 32)
		h256.Read(res)
		if bytes.Compare(res, []byte{
			0x9d, 0xd2, 0xfe, 0x4e, 0x90, 0x40, 0x9e, 0x5d,
			0xa8, 0x7f, 0x53, 0x97, 0x6d, 0x74, 0x05, 0xb0,
			0xc0, 0xca, 0xc6, 0x28, 0xfc, 0x66, 0x9a, 0x74,
			0x1d, 0x50, 0x06, 0x3c, 0x55, 0x7e, 0x8f, 0x50,
		}) != 0 {
			t.Fatalf("Want %x, has %x", []byte{
				0x9d, 0xd2, 0xfe, 0x4e, 0x90, 0x40, 0x9e, 0x5d,
				0xa8, 0x7f, 0x53, 0x97, 0x6d, 0x74, 0x05, 0xb0,
				0xc0, 0xca, 0xc6, 0x28, 0xfc, 0x66, 0x9a, 0x74,
				0x1d, 0x50, 0x06, 0x3c, 0x55, 0x7e, 0x8f, 0x50,
	...
	t.Run("Перун", func(t *testing.T) {
		// It is "половци идуть отъ Дона
		// и отъ моря,
		// и отъ всехъ странъ рускыя плъкы оступиша.
		// Дети бесови кликомъ поля прегородиша,
		// а храбрии русици преградиша чрълеными щиты." string
		h512.Reset()
		m := []byte("половци идуть отъ Дона и отъ моря, и отъ всехъ странъ рускыя плъкы оступиша. Дети бесови кликомъ поля прегородиша, а храбрии русици преградиша чрълеными щиты.")
		h512.Write(m)
		t.Log(hex.EncodeToString(h512.Sum(nil)))
	})
